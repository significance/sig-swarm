...
browser or node
app developer / user centric

As an educational exercise, we'll quickly walk through these options and explain what each means and why we are using the default settings in this case. Often you will need to specify different configurations, and swarm.js is deceptively nuanced and powerful, it can allow very fine granularity of control, which also ensuring the uninitiated users a beautifully elegant and frictionless onboarding experience ;D

lowlevel,
primitives, data manipulation

median
with transport, encryption, collections, blockchain, etc

high
abstractions, accounts, ORM, collaborative databases
...
#### HTTP Gateway's and Bee Proxies
...
We can now redefine HTTP Gateways as "edge nodes" which are responsible for providing services that for individuals or applications that have so far failed to overcome the hurdle of becoming truly p2p and running a bidirectional wire protocol. One could conceive of these nodes being more profitable than normal Bee nodes, perhaps charging a premium for forwarding services, or by providing edge caching services for content specific to an application or purpose, for example open street maps tiles.

The Admin API is very simply just a means of interacting with the Bee software. When a Bee node is run, users may interact with it using command line networking tools such as curl, a http gui api, or other applications running in a local network.
...
However, this is no silver bullet. Each of these services will need a reimagined endpoint to satisfy the different end user requirements created by the nature of the environments that our javascript code will likely be run in.

#### Javascript User Environment Targets

##### Node.js

Node.js is a javascript runtime that runs *in userspace on a computer*.

This is fundamentally important because it is a totally different environment to that of a browser. The Node.js environment benefits from all of the same priveleges as any other software running natively on a computer such as access to incoming and outgoing network ports, unabated access to the filesystem, and so on. It also has completely different API's to handle types such as the *Buffer* object which is a more sane way approach to native javascript's *ArrayBuffer* and associated types. Some familiar objects from the browser are totally missing, of course there is no *window* or other DOM elements, but futhermore the *File* and *Localstorage* api's are missing. This list is by no means exhaustive!

##### Browser

Web browsers run a *sandboxed* runtime which is *mostly* isolated for each tab or iframe.

Browsers have a *very limited* set of privileges designed to improve security for web users. Javascript runtimes in a browser are prevented from access to entities such as the file system and networking ports.

Despite, or perhaps because of this, browsers do have a rich and diverse set of API's that node does not. WebRTC provides p2p wire protocols via STUN and TURN servers. Work to get Libp2p to use this is ongoing. File objects can be created using input html entities, and so on...

Because browsers do not have access to block filestorage, it is not possible to use the environment to directly store large amounts of data in a reliable or sane manner. The browser sandbox environment does typically have access to limited space to be able to store data. Storing data in Localstorage, IndexDB, volatile memory and (?) is all possible to a certain extent. This means that using Browser environment runtimes to administrate long term storage nodes is infeasible.

In many environments bandwidth may also be limited, and this would make the nodes purely consumers and remuniration of the gateways would be necessitated by other means.

In further environments computational power or volatile memory itself would also be short supply and this should be considered.
